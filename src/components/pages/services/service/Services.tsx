...
const Services = () => {
  // IMAGES COMPLETELY REMOVED FROM DATA
  const servicesData = [
    { id: 1, category: "01. Pre-Visit", title: "Insurance Verification", desc: "Ensure 100% financial clarity before the patient sits in the chair. We catch potential downgrades and frequency limitations instantly to prevent unexpected out-of-pocket costs.", list: ["Updated breakdowns uploaded", "Accurate patient estimates", "Essential codes verified", "Real-time eligibility checks"] },
    { id: 2, category: "02. Operations", title: "Appointment Scheduling", desc: "Keep your chairs full and your production high without the front-desk overwhelm. We manage your calendar with precision to ensure a steady flow of patients.", list: ["Inbound call handling", "Outbound recall", "Schedule optimization", "New patient intake"] },
    { id: 3, category: "03. Pre-Approval", title: "Prior Authorizations", desc: "Eliminate the risk of performing major procedures that won’t be covered. We handle the clinical back-and-forth and documentation submission to secure approvals.", list: ["Clinical Narrative review", "Attachment Management", "Relentless Payer Follow-up", "Status Updates"] },
    { id: 4, category: "04. Post-Visit", title: "Claim Processing", desc: "Doctor-led claim submission designed for maximum first-pass approval and zero revenue leaks. We treat every claim with clinical precision.", list: ["Review denied claims", "Troubleshoot missing info", "Attach required documents", "Daily EOBs posted"] },
    { id: 5, category: "05. Collections", title: "Patient Billing", desc: "Consistent, professional follow-ups that protect your patient relationships while maximizing your practice’s cash flow through accurate accounting.", list: ["Patient Ledger Audits", "Personalized Letters", "Outbound Phone Calls", "Electronic Statements"] },
    { id: 6, category: "06. Revenue Recovery", title: "A/R Recovery", desc: "Targeting outstanding insurance balances aged 30, 60, or 90+ days to turn 'stuck' claims into liquid revenue. We systematically clean up your aging report.", list: ["Aged Balance research", "Aggressive Appeals", "Insurance Follow-up", "Monthly Reporting"] },
    { id: 7, category: "07. Compliance", title: "Dental Credentialing", desc: "Keep your providers in-network and your practice fully compliant without the administrative headache. We manage the entire enrollment lifecycle.", list: ["Complete applications", "Attach documentation", "Monitor progress", "In-network pricing"] },
    { id: 8, category: "08. Security", title: "Security & Compliance", desc: "Aligning your practice with the Compliance & Operations Binder standards. We ensure your back-office operations meet all federal and state requirements.", list: ["SRA Support via Health IT", "MSA & BAA templates", "Subcontractor tracking", "Audit Readiness logging"] },
  ];

  return (
    <section className="pt-120 pb-120 position-relative overflow-hidden" style={{ backgroundColor: "#FAFAFA" }}>
      <div className="container-fluid px-4 px-xl-5 position-relative z-index-1">
        ...
        <div className="row justify-content-center text-center mb-80">
          <div className="col-lg-8 col-xl-6">
            ...
            <h2 className="fw-black mb-4" style={{ color: "#003941", fontSize: "clamp(2.5rem, 4vw, 3.5rem)", lineHeight: "1.1", letterSpacing: "-1px" }}>
              Expert Dental Billing <br className="d-none d-md-block" /> <span style={{ color: "#09B2AB" }}>Solutions</span>
            </h2>
            <p className="mx-auto" style={{ color: "#002C34", fontSize: "1.1rem", lineHeight: "1.7", opacity: 0.85 }}>
              Financial health is the backbone of a thriving practice, but clinical nuances shouldn't be your daily distraction.
            </p>
          </div>
        </div>

        {/* Card Grid - NO IMAGES, NO LAG */}
        <div className="row g-4 g-xl-5 justify-content-center">
          {servicesData.map((item) => (
            <div key={item.id} className="col-xl-3 col-lg-4 col-md-6 d-flex flex-column">
              <div className="w-100 rounded-4 overflow-hidden position-relative shadow-sm d-flex flex-column" style={{ minHeight: "420px", border: "1px solid rgba(0,0,0,0.05)", backgroundColor: "#002C34", transition: "transform 0.2s ease" }}
                   onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-5px)"; }}
                   onMouseLeave={(e) => { e.currentTarget.style.transform = "translateY(0)"; }}>
                
                {/* Clean, simple card content */}
                <div className="position-relative z-index-2 p-4 d-flex flex-column h-100" style={{ zIndex: 2 }}>
                  <div className="mb-auto">
                    <span className="badge px-3 py-2 rounded-pill shadow-sm" style={{ backgroundColor: "rgba(255,255,255,0.1)", color: "#ffffff", border: "1px solid rgba(255,255,255,0.2)" }}>
                      {item.category}
                    </span>
                  </div>
                  <div className="mt-auto pt-4">
                    <h4 className="fw-bold mb-3" style={{ color: "#ffffff", fontSize: "1.3rem" }}>{item.title}</h4>
                    <p className="mb-3" style={{ color: "#E5EAEB", fontSize: "0.9rem", lineHeight: "1.5" }}>{item.desc}</p>
                    <ul className="list-unstyled mb-0 border-top pt-3" style={{ borderColor: "rgba(255,255,255,0.15)" }}>
                      {item.list.map((listItem, i) => (
                        <li key={i} className="d-flex align-items-start mb-2" style={{ color: "#ffffff", fontSize: "0.85rem" }}>
                          <i className="fa-solid fa-check mt-1 me-2" style={{ color: "#09B2AB", fontSize: "10px" }}></i> {listItem}
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <div className="mt-4 ms-2 d-flex">
                <Link to="/contact" className="d-inline-flex align-items-center fw-bold shadow-sm rounded-pill px-4 py-2 text-uppercase" style={{ backgroundColor: "#002C34", color: "#ffffff", fontSize: "12px", textDecoration: "none", transition: "background-color 0.2s" }}
                      onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = "#09B2AB"; }}
                      onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "#002C34"; }}>
